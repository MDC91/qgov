...
interface PageProps {
  params: Promise<{ epoch: string; id: string; lang: string }>;
}
...
export default async function Page({ params }: PageProps) {
  const { epoch: epochStr, id: slugOrId, lang } = await params;
  ...
  return (
    <div className="min-h-screen text-white" style={{ backgroundColor: '#101820' }}>
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        <ProposalDetail epoch={epoch} id={proposal.id} initialProposal={proposal} initialLang={lang} />
      </main>
    </div>
  );
}
